...
var set_localAIbaseline_dataGrid
...
{
    const elements = document.getElementsByClassName("my-popover")
...
async function baselineTableWithFeaturesHTML_REALLY(numView = 1)
{
   ...
   let s = `
        ...
        <br>
        <br>
        <button onClick='window.LocalAIbaseline.viewDataGridHTML()' style='padding:6px;border-radius:10px'>View Data Grid</button>
...
async function viewDataGridHTML(reload)
{
   lastGridViewed = "viewDataGridHTML"
    document.getElementById("localAIbaselineFeaturesTable").innerHTML =

        "<br><button onClick='window.LocalAIbaseline.baselineTableWithFeaturesHTML()' style='padding:6px;border-radius:10px'>View Browser Compatibility Grid</button>"
        + " &nbsp; &nbsp; <button onCLick='window.LocalAIbaseline.viewDataGridHTML(true)' style='background-color:#fdd'>reload</button>"
        + "<br><br>"
...
{
    if(!set_localAIbaseline_dataGrid || reload)
       await downloadDataGrid()

    if(!set_localAIbaseline_dataGrid)
        return "<b style='color:red'>file with data not found or wrong version</b>"

    let s = "<table border='1'><tr><th>num</th><th>LLM ID</th><th>Model ID</th><th>type</th><th>baseline</th>"
                ...
                + "</tr>"
    for(let llmmodelTypeToMinMax of set_localAIbaseline_dataGrid)
       s += "<tr><td style='text-align:right'>&nbsp;" + llmmodelTypeToMinMax.numData + " <button onClick='MyLLMroot.calculateMenu_llm_model(\"" + llmmodelTypeToMinMax.llmID + "\",\"" + llmmodelTypeToMinMax.modelID + "\",\"" + llmmodelTypeToMinMax.type +"\")'>+</button>&nbsp;</td>"
           ...
           + "<td><input type='text' disabled value='" + llmmodelTypeToMinMax.baseline_features + "'></td>"
           ...
           + "<td>" + llmmodelTypeToMinMax.averageFirstCharDuration_0.toFixed(1) + "<br>" + llmmodelTypeToMinMax.minFirstCharDuration_0  + " &nbsp; " + llmmodelTypeToMinMax.maxLoadingDuration_0
           ...
           + "<td>" + llmmodelTypeToMinMax.averageFirstCharDuration_1.toFixed(1) + "<br>" + llmmodelTypeToMinMax.minFirstCharDuration_1  + " &nbsp; " + llmmodelTypeToMinMax.maxLoadingDuration_1
...
{
const url = "https://storage.googleapis.com/localaibaseline/local_localaibaseline.csv"

  const res = await fetch(url, { cache: "no-store" }); // or "force-cache"
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
  const s = await res.text();

  set_localAIbaseline_dataGrid = new Set()
  let lastPos = 0
  let lastPosGlobal = 0
    while(lastPosGlobal < s.length)
    {
        ...
        const key = line.slice(lastPos, pos)
        lastPos = pos +1
        let pos2 = key.indexOf(' ')
        let pos3 = key.indexOf(' ', pos2 + 1)
        let pos4 = key.indexOf(' ', pos3 + 1)
        ...
        llmmodelTypeToMinMax.baseline_features = key.slice(pos4)
        ...
        set_localAIbaseline_dataGrid.add(llmmodelTypeToMinMax)
...
//-----------------------------------------------------------------
function performanceGrid(numView, uuid = "")
{
    const type = ModelOfMyLLMroot.selectedModelType
...
function menuSelectModelType(numView = 0, uuid = "")
{
    ...
    for(let n = 0; n < titles.length; n++)
        s += "<label> &nbsp; <input onClick='window.LocalAIbaseline.performanceGridHTML(" + n + ")' type='radio' "+ (numView === n ? "checked" : "") +" name='radios_engines_models' style='margin-bottom:6px'>&nbsp;" + TLtranslateFromTo(titles[n]) + "&nbsp;</label>"
...
class MyLLMroot
{
    ...
    constructor(uniqueID, name, imageURL, siteURL = "https://aimagazine.com/articles/top-10-ai-cloud-platforms")
    {
        ...
        this.translationIsDoneByChat = true //when has specific way to do Translation then places "false"
...
//----------------------------------------
static async calculateMenu_llm_model(llm_uniqueID, modelOfMyLLM_id, type)
{
...
let s = "<table style='width:100%'><tr><th rowspan='2' style='width:1px'>" + llm_selected.icon() + "</th><th style='text-align:left'>"+ llm_selected.name +"</th><th>&nbsp;+&nbsp;</th><th style='text-align:right'>" + model_selected.name + "</th><th rowspan='2' style='width:1px'>" + model_selected.icon("30px") + "</th></tr>"
                + "<tr><td colspan='3' style='color:green'>operation = " + ModelOfMyLLMroot.selectedModelType + "</td></tr>"
                + "</table>"
...
//---------------------------------
static async post_LLM_Model_Type_arrDuration_arrloading_arrFirstChar_arrProcessing(baseline_features
...
static async get_agent_device_os(first = true)
{
     const url = location.protocol + "//" + (location.hostname === "localhost"
                                                    ? "api.localaibaseline.localhost:" + location.port
                                                    : "api.localaibaseline.com") + "/get_agent_device_os";
