...
function menuSelectModelType(numView = 0, uuid = "") {
    ...
    for (let [name, llm] of MyLLMroot.mapMyLLMs) {
        ...
        for (let [name, modelOfMyLLM] of ModelNameLLM.mapNameToModels)
            s += "<td class='numModelsOfLLM_modelOfMyLLM_" + llm.uniqueID + "_" + modelOfMyLLM.name + "'>" + (llm.numModelsOfLLM(modelOfMyLLM) || "") + "</td>"
        s += "</tr>"
    }
    ...
    for (let [name, modelOfMyLLM] of ModelNameLLM.mapNameToModels) {
        ...
        for (let [name, llm] of MyLLMroot.mapMyLLMs)
            s += "<td class='table_with_all_llm_" + llm.uniqueID + "' style='background-color:" + llm.rowColor() + "'>" + (llm.numModelsOfLLM(modelOfMyLLM) || "") + "</td>"
